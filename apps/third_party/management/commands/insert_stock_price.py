        ...
        if options['stock_code'] is None:
            for stock in _get_stock_list():
                try:
                    ...
                    fd.register(stock['stocks_id'], stock['stock_code'], stock_name = stock['stock_name'])
                except:
                    continue
...
def _get_stock_list():
    ...
    portfolio_list = PortfoliosDetail.objects.values('stocks_id__stock_code', 'stocks_id__stock_name').annotate(stock_count = Count('stocks_id'), stocks_id = F('stocks_id'), )
    for stock in portfolio_list:
        stock_list.append({ 'stocks_id': stock['stocks_id'], 'stock_code': stock['stocks_id__stock_code'], 'stock_name': stock['stocks_id__stock_name'] })

    subs_list = StockSubs.objects.values('stocks_id', 'stocks_id__stock_code', 'stocks_id__stock_name').all()
    for stock in subs_list:
        stock_list.append({ 'stocks_id': stock['stocks_id'], 'stock_code': stock['stocks_id__stock_code'], 'stock_name': stock['stocks_id__stock_name'] })
